...
(defun magit-gerrit-remove-reviewer ()
  (interactive)
  "ssh -x -p 29418 user@gerrit gerrit set-reviewers --project toplvlroot/prjname -r email@addr"

  (gerrit-ssh-cmd "set-reviewers"
                  "--project" (magit-gerrit-get-project)
                  "-r" (read-string "Reviewer Name/Email: ")
                  (cdr-safe (assoc 'id (magit-gerrit-review-at-point)))))
...
(defun magit-gerrit-push-review (status &optional args)
  (let* ((branch (or (magit-get-current-branch)
    ...
    (let* ((branch-merge (if (or (null branch-remote)
      ...
      (magit-run-git-async "push" "-v" branch-remote
                           (concat rev ":" branch-pub)
                           (if (--any? (string= "--no-verify" it) args)
                               "--no-verify")))))
...
(magit-define-popup magit-gerrit-popup
  ...
  :actions '((?P "Push Commit For Review"                          magit-gerrit-create-review)
             ...
             (?R "Remove Reviewer"                                 magit-gerrit-remove-reviewer)
  ...
  :switches '((?h "Disable pre-push hooks"      "--no-verify")))
